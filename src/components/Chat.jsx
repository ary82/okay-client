...
export default function Chat({ to }) {
  ...
  const getConversation = () => {
    axios({
      method: "get",
      url: `https://okayapi.ary82.dev/conversation/${user.username}/${to}`,
    })
      .then((res) => {
  ...
  const getRoom = () => {
    axios({
      method: "get",
      url: `https://okayapi.ary82.dev/room/${user.username}/${to}`,
    })
      .then((res) => {
  ...
  const postAImessage = () => {
    ...
    axios({
      method: "post",
      url: "https://okayapi.ary82.dev/ai",
  ...
  const postMessage = (e) => {
    ...
    axios({
      method: "post",
      url: "https://okayapi.ary82.dev/message",
  ...
  useEffect(() => {
    if (to != "") {
      ...
      socket.on("receive-message", (m, id, from) => {
        setsocketMessages((oldsocketMessages) => [
          ...oldsocketMessages,
          {
            from,
            message: m,
        ...
        <div className="grow basis-3/4 bg-slate-900 overflow-y-auto ">
          <ul className="flex flex-col gap-4 p-4 pb-24">
            {conversation
              .slice()
              .reverse()
              .map((chat, index, convo) => (
                <li className="relative flex flex-col" key={chat._id}>
                  <Message
                    chat={chat}
                    index={index}
                    convo={convo}
            ...
            {socketMessages.map((m, index, convo) => (
              <li className="relative flex flex-col" key={crypto.randomUUID()}>
                <Message chat={m} index={index} convo={convo} isSocket={true} />
              </li>
            ))}
          </ul>
